                ...
                {{-- <div class="sidebar-widget widget-category-2 mb-30">
                    ...
                    <ul>
                        @foreach ($categories as $item)
                            <li>
                                <a href="shop-grid-right.html"> <img src="{{asset($item->category_image_logo)}}" alt="{{$item->category_slug}}" />{{$item->category_name}}</a><span class="count"></span>
                            </li>
                        @endforeach
                    </ul>
                </div> --}}
                    ...
                    <div class="list-group">
                        <div class="list-group-item mb-10 mt-10">
                            ...
                            <div class="custome-checkbox">
                                @foreach ($categories as $item)
                                @php
                                    $products = App\Models\Product\Product::where('category_id',$item->id)->get();
                                @endphp
                                <input class="form-check-input" name="category[]" type="checkbox" name="checkbox" id="exampleCheckbox{{$item->id}}" value="{{$item->category_slug}}" @if(!empty($filterCat) && in_array($item->category_slug,$filterCat)) checked @endif onchange="this.form.submit()" />
                                <label class="form-check-label" for="exampleCheckbox{{$item->id}}"><span>{{$item->category_name}} ({{ count($products) }})</span></label>
                                <br />
                                @endforeach
                            </div>
                            @if(!empty($_GET['brand']))
