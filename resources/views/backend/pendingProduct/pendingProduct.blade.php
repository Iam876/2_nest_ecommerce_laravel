							...
							<table id="example" class="table table-striped table-bordered MyDataTable" style="width:100%">
								...
								<tbody class="">
									...
									@if (count($pending_order)>0)
										@foreach ($pending_order as $pendingOrder)
											<tr>
												<td>{{$no++}}</td>
												...
												<td><span class="btn btn-outline-primary btn-sm">{{$pendingOrder->status}}</span></td>
												<td>
													<a href="{{Route('pending.order',$pendingOrder->id)}}" class="btn btn-primary">EDIT</a>
												</td>
											</tr>
										@endforeach
									@else
