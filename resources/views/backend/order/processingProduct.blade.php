							...
							<table id="example" class="table table-striped table-bordered MyDataTable" style="width:100%">
								...
								<tbody class="">
									@php
										$no = 1;
									@endphp
									@if (count($processing_order)>0)
										@foreach ($processing_order as $processingOrder)
											<tr>
												<td>{{$no++}}</td>
												<td>{{$processingOrder->order_date}}</td>
												<td>{{$processingOrder->invoice_no}}</td>
												<td>{{$processingOrder->amount}}</td>
												<td>{{$processingOrder->payment_type}}</td>
												<td><span class="btn btn-outline-primary btn-sm">{{$processingOrder->status}}</span></td>
												<td>
													<a href="{{Route('processing.order',$processingOrder->id)}}" class="btn btn-primary">EDIT</a>
												</td>
											</tr>
										@endforeach
									@else
