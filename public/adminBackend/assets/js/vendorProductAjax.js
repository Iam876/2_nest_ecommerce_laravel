$(document).ready(function(){
            ...
            success: function(response){
                if(response.status == 200){
                    ...
                    $.each(response.AllProducts,function(key,val){
                        ...
                        <td>'+status_output+'</td>\
                        <td>\
                        <a href="'+'/edit/product/vendor/'+val.id+'}}" class="btn btn-secondary btn-sm"><i class="font-22 fadeIn animated bx bx-edit-alt"></i></a>\
    ...
    $(document).on("click","#Inactive",function(){
        var id = $(this).val();
        $.ajax({
            url:"/inactive/product/vendor/"+id,
            type:"GET",
            success: function(response){
