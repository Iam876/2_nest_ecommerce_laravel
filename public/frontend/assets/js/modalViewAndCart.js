   ...
   $(document).on("click","#ProductModalShow",function(){
    ...
    $.ajax({
        ...
        success: function(response){
            var amount = response.product.selling_price - response.product.discount_price;
            var discount = Math.round((amount / response.product.selling_price)*100);
            ...
            if(response.product.discount_price === null){
                $("#ModalSellingPrice").text("$"+response.product.selling_price)
            }else{
                $("#ModalSellingPrice").text("$"+response.product.discount_price)
                $("#ModalDiscountPercent").text(discount+"% Off");
                $("#ModalDiscountPrice").text(response.product.selling_price);
            }
            ...
            $("#ModalSubCategory").text(response.product.subcategory.subcategory_name);
            $("#ModalMFG").text(" "+year+"-"+month+"-"+day);
            $("#pimage").attr("src", '/' + response.product.product_thumbnail);

            var Tagshtml = '';
            response.tags.forEach(function(tag){
                Tagshtml+='<a href="#" rel="tag"  value="'+tag+'">'+tag+' </a>'
            });
            ...
            if(response.product.product_color === null){
                ...
                response.colors.forEach(function(color) {
                    ...
                    ProductColor += '<a href="#" data-value="' + color + '">' + color + '</a>';
                    ProductColor += '</li>';
                });
            ...
            if(response.product.product_size === null){
                ...
                response.sizes.forEach(function(size) {
                    ...
                    ProductSize += '<a href="#" data-value="' + size + '">' + size + '</a>';
                    ProductSize += '</li>';
                });
